...
本版本在已锁定的 `v0.3.6` 基础上，重点提升大素材库备份、AI 接口兼容性和发布交付可靠性：

**新增**

- 大型素材库自动按容量分卷导出，支持一次选择多卷导入，并共享分类、标签及封面资源。
- 导出任务显示阶段进度并在后台执行，导入 / 导出目录记忆上次使用位置，减少大文件备份等待。
- OpenAI 兼容接口支持非流式、SSE、NDJSON、UTF-8 BOM 及常见响应包装，提升图像反推和 AI 分析成功率。
- 增加脱敏 AI 诊断日志，便于定位模型、网关、响应格式和下载阶段的问题，不记录密钥或图像内容。
- 补充 ComfyUI 快捷保存与 TapRelay 生图完成通知的使用说明和配套截图。
- 提供四种 Windows 发布包，附 SHA-256 校验和、发布验证清单及双语 Release 说明。

**优化**

- 延续创作画布自适应展示、主题跟随背景、参数侧栏和生图动效。
- 延续模型列表使用接口实际模型名称，并记忆各操作的默认模型与分析来源。
- 延续分类 / 标签归纳整理、同义词合并、实体分组和错误分组约束。
- 延续图片复制、缩略图回退、原图顺序，以及导出进度、后台导出和目录记忆。
- 延续毛玻璃导航、多角色主题色、响应式布局、统一弹窗与安装 / 卸载界面。
- 延续日志脱敏、错误提示、数据目录保护和正式包安全检查。
