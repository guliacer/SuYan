...
- 增加 Ollama 本地模型适配，可在本机使用无需 API Key 的模型完成 AI 分析；模型列表和能力状态会在模型设置中展示。
...
<img src="./docs/assets/v0.4.0-ollama.png" alt="Ollama 本地模型适配" width="100%" />
...
- 已生成作品区会保留不同提示词批次的全部图像，最新生成结果自动置顶，作品较多时可在区域内上下滚动查看。
...
<img src="./docs/assets/v0.4.0-canvas.png" alt="创作画布与已生成作品区域" width="100%" />
...
安装并启用 [ComfyUI-GuliNodes](https://github.com/guliacer/ComfyUI-GuliNodes) 后，ComfyUI 生图完成时，点击画布上方的“素言”图标，即可将生成的图像和对应提示词快捷发送到素言保存。

首次使用前请先启动素言并保持软件运行。发送成功后，图像会进入素言素材库，提示词、负向提示词、标题和生成方式会一并写入；ComfyUI PNG 中包含的 prompt / workflow 元数据也会自动解析。

如果发送失败，请确认素言正在运行、ComfyUI 可以访问 `127.0.0.1:9477`，并检查 ComfyUI-GuliNodes 是否安装并启用。

<img src="./photo/readme/comfyui-quick-save.png" alt="ComfyUI 点击素言图标快捷保存" width="100%" />

<img src="./photo/readme/comfyui-saved-detail.png" alt="素言中已保存的 ComfyUI 图像与提示词" width="100%" />

#### 生图完成通知

需要离开电脑时，可以使用 [TapRelay](https://github.com/guliacer/TapRelay/releases) 将生图完成状态通知到手机。先下载并按 TapRelay 的说明完成设备连接，再回到素言的创作画布，在开始生图前打开画布下方的通知按钮。

生图任务完成后，素言会通过 TapRelay 向已连接的手机发送完成通知；通知内容会包含任务完成状态和基本任务信息。使用前请保持 TapRelay 可用，并确认手机与电脑之间的连接正常。

<img src="./photo/readme/generation-notification-toggle.png" alt="在素言画布中开启生图完成通知" width="100%" />

<img src="./photo/readme/generation-notification-phone.jpg" alt="手机收到素言的生图完成通知" width="100%" />
...
- 查看图像时增加克制的粒子视觉效果；弹窗提示、负向提示词提取、文档换行识别和系统设置布局也已同步优化。
...
- **Ollama 本地模型 AI 分析**：增加 Ollama 本地模型适配，可在本机完成图像和提示词分析，模型设置中可查看模型能力和连接状态。
- **图像查看粒子效果**：为页面查看图像增加克制的粒子视觉效果，增强作品浏览的空间感。
- **已生成图像展示区**：创作画布右侧增加作品展示区，完整保留不同提示词批次的结果，最新批次置顶、旧结果向下排列，并支持区域内滚动查看、隐藏和宽度调整。
- **版本引导**：新安装和版本升级后显示一次版本引导，帮助用户了解本次变化和升级前的数据备份提醒。

**功能优化**

- 优化部分弹窗反馈文案，让提示更自然、轻松且不影响判断。
