    ...
    <section className="flex flex-col overflow-hidden rounded-xl border border-primary/30 bg-panel shadow-sm">
      <header className="flex shrink-0 flex-wrap items-center justify-between gap-3 border-b border-primary/25 bg-primary-soft px-4 py-2 text-primary">
        ...
        {durationLabel ? (
          <span className="inline-flex min-h-6 items-center rounded-lg border border-primary/25 bg-panel px-2 text-xs font-medium tabular-nums text-primary">
            {t("时长 {duration}", { duration: durationLabel })}
          </span>
        ) : null}
      </header>
      ...
      <div className="flex flex-col gap-2">
        <div className="relative flex items-center justify-between gap-2">
          <span className="text-xs font-medium text-muted">{t("参考素材")}</span>
          <Button
          ...
          >
            {isImporting ? t("导入中") : t("导入素材")}
          </Button>
          {isImportMenuOpen ? (
            <>
              <button
                aria-hidden="true"
                className="app-window-overlay z-10 cursor-default"
                tabIndex={-1}
                type="button"
          ...
          <div className="grid grid-cols-[repeat(auto-fill,minmax(72px,1fr))] gap-2">
            {referenceImages.map((referenceImage, index) => {
              ...
              return (
                ...
                >
                  <button
                    aria-label={t("{action}参考素材 {index}", { action: t(isAudio ? "试听" : "查看"), index: index + 1 })}
                  ...
                  >
                    {isAudio ? (
                      <span className="flex size-full flex-col items-center justify-center gap-2 bg-primary-soft text-primary">
                        <Music2 size={24} />
                        <span className="text-[11px] font-medium">{t("音频")}</span>
                      </span>
                    ) : (
                      <img
                        alt={t("参考素材 {index}", { index: index + 1 })}
                  ...
                  <button
                    aria-label={t("删除参考素材 {index}", { index: index + 1 })}
      ...
      <div className="flex flex-col gap-2">
        <div className="flex items-center justify-between gap-2">
          <span className="text-xs font-medium text-muted">{t("关键帧时间轴")}</span>
          <Button
            className="min-h-8 gap-1.5 rounded-lg px-3 text-xs"
            disabled={busy}
        ...
        {hasKeyframes ? (
          <div className="grid grid-cols-[repeat(auto-fill,minmax(96px,1fr))] gap-2">
            {keyframes.map((keyframe, index) => (
              ...
              >
                <button
                  aria-label={t("查看关键帧 {label}", { label: keyframe.label })}
